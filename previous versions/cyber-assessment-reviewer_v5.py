...
for dir_name in ['uploads', 'sessions', 'static', 'templates', 'models']:
...
class CyberAssessmentReviewer:
    def __init__(self, model_name="mistralai/Mistral-7B-Instruct-v0.3", use_quantization=True):
        """
        Initialize with Mistral 7B for optimal cyber assessment analysis
        """
        logger.info(f"Initializing Cyber Assessment Reviewer with {model_name}")
...
<body>
    <header>
        ...
        <p class="model-info">Powered by Mistral 7B Instruct - Running Locally</p>
    </header>
    ...
    <div class="container">
        ...
        <div id="loadingSection" class="loading">
            ...
            <h3>Analyzing Assessment with Mistral 7B</h3>
...
# Updated requirements file with Mistral 7B dependencies
...
- For CPU-only: Expect slower performance but still functional
...
- The first run will download the Mistral 7B model (~13GB)
- This may take 10-30 minutes depending on your internet speed
- The model will be cached locally for future use

## Usage Guide:

1. **Select Framework**: Choose your compliance framework (NIST, ISO27001, etc.)
2. **Upload Assessment**: Upload your Excel file with cyber controls
3. **Upload Evidence**: Add supporting documents (PDFs, presentations, etc.)
4. **Configure Analysis**: Set the number of controls to analyze
5. **Run Analysis**: Click "Analyze Assessment" and wait for results
6. **Review Results**: Examine risk scores, compliance status, and recommendations
7. **Download Report**: Get a comprehensive Excel report with all findings

## Performance Optimization:

### For GPU Users:
- The system automatically uses 4-bit quantization for efficient VRAM usage
- Expect 5-10 seconds per control analysis

### For CPU Users:
- Analysis will be slower (30-60 seconds per control)
- Consider reducing max_controls to 10 for faster results
- Close other applications to free up RAM

## Excel Assessment Format:

Your assessment Excel file should contain columns like:
- `control_id` or `control` - Unique identifier
- `control_name` or `name` - Control description
- `requirement` or `description` - What the control requires
- `answer` or `response` - Supplier's response
- `status` - Implementation status (optional)

## Customization Options:

### Change Model (in app.py):
```python
# For smaller model (8GB RAM):
model_name = "mistralai/Mistral-7B-Instruct-v0.1"

# For larger model (32GB RAM):
model_name = "mistralai/Mixtral-8x7B-Instruct-v0.1"
```

### Adjust Analysis Depth:
- Modify prompts in `create_cyber_prompt()` method
- Add custom frameworks to `self.frameworks` dict
...
- Reduce number of controls analyzed
- Disable detailed evidence extraction

### Model Download Issues:
- Check internet connection
- Manually download from Hugging Face
- Use alternative model like Phi-2 for testing

## Security Notes:

- All processing happens locally - no data leaves your machine
- Session data is stored temporarily in ./sessions/
- Clear sessions folder periodically for privacy
- Change Flask secret key in production

## Support:

For issues or questions:
1. Check model is downloaded correctly
2. Verify CUDA installation (for GPU)
